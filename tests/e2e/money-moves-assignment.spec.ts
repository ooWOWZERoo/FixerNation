...
// ---------------------------------------------------------------------------
// Money Moves — catalog scaling slice (Challenge band, Simulation engine).
...
test("teacher assigns Money Moves, student plays it, a real score is recorded", async ({ page }) => {
  ...
  const moneyMoves = (await games.json()).games.find((g: any) => g.slug === "money-moves");
  expect(moneyMoves).toBeTruthy();

  const detail = await page.request.get(`/api/classrooms/${classroom.id}`);
  const existing = (await detail.json()).gameAssignments.find((ga: any) => ga.game_slug === "money-moves");
  ...
  await page.goto(`/student-game.html?assignment=${assignmentId}&slug=money-moves&name=Money+Moves`);
  ...
  // Simulation is intentionally scoreless; duration_ms is the real,
  // non-null proof the auto-report fired.
