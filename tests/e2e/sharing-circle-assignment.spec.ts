...
// ---------------------------------------------------------------------------
// Sharing Circle — Discover's first Social awareness / Relationship
// skills game (2 of Discover's 3 missing CASEL competencies before this
...
test("teacher assigns Sharing Circle, student plays it, a real score is recorded", async ({ page }) => {
  ...
  const sharingCircle = (await games.json()).games.find((g: any) => g.slug === "sharing-circle");
  expect(sharingCircle).toBeTruthy();

  const detail = await page.request.get(`/api/classrooms/${classroom.id}`);
  const existing = (await detail.json()).gameAssignments.find((ga: any) => ga.game_slug === "sharing-circle");
  ...
  await page.goto(`/student-game.html?assignment=${assignmentId}&slug=sharing-circle&name=Sharing+Circle`);
