...
The admin page's new **Activity Log** tab reads it back through `GET /api/affiliates/audit-log`, filtered to the affiliate program's own `entity_type` values (`affiliate`, `affiliate_application`, `affiliate_commission`, `affiliate_territory`) so it never surfaces an unrelated school-admin row.

**The reverse direction wasn't true on first pass, and got caught by checking rather than assuming.** `school-admin.js` has three purchase-scoped audit views (a "recent activity" excerpt, a full activity report, and a paginated audit history a school license admin can view themselves), and none of them filtered by `entity_type` at all — only `purchase_id`. A commission tied to a purchase (every referral-sourced one is) would show up in all three the moment that same purchase also happened to be a `group_license` sale with a school admin assigned to it. The `pay` transition's audit entry carries a `payout_reference` — a real payment reference — in `new_value`, which the third view (the school-admin-facing one, not just an FNE-staff one) renders directly. That's a real leak of one program's sensitive data into an unrelated admin's screen, not a cosmetic overlap.

Fixed by sharing one list both directions: `AFFILIATE_ENTITY_TYPES`, now exported from `lib/audit.js` rather than declared locally in `routes/affiliates.js`. All three `school-admin.js` queries gained an explicit `entity_type NOT IN (...)` against that same list. The claim in this doc — "nothing written here appears on school-admin's own views" — is only true now that this exists; it wasn't true for the first draft of 3.5c, and this addendum says so rather than quietly fixing it and leaving the original overclaim standing.
