...
## Scenario Choice engine + Feelings Detective — built, not yet deployed

See `SCENARIO_CHOICE_ENGINE_SPIKE.md` for the full spike and what was decided. Summary: no existing engine supports multi-select (every one of the 3 choice-based engines locks in a single choice on first click), which is exactly what genuine Self-awareness content (the "Feelings Detective" concept, §14.1) requires — the acceptance rule explicitly allows more than one plausible feeling per scenario.

**`engine-scenario-choice.js`** (registered as `scenarioChoice`) implements a two-stage-per-scenario shape reusing Evidence Hunt's proven pattern (notice cues, then pick feelings), both stages multi-select, narrated from the start (reuses Audio Choice's `speak()`/replay pattern), scoreless `onComplete` per §5.6. The SDK contract itself (`engine-sdk.js`) needed zero changes.

**Feelings Detective** (`brain-feelings-detective.html`, Discover band) — 4 scenarios (a friend not saving a seat, a new puppy, a lost game, starting at a new school), each tagging 2 plausible feelings (never just 1) plus one `less_fitting` option with a cue-based redirect note, not a "wrong" framing. 2 of the 4 scenarios add a seek-help beat, per the acceptance rule's "includes help-seeking in appropriate situations" — deliberately not all 4. New badge **Cue Spotter** 🔍. This closes the last CASEL gap — every one of the 5 competencies now has coverage in at least one band.

**Real bug found and fixed during local verification, not by code review alone**: Discover-band's global choice-button CSS assumes a single emoji glyph (60px font, grid layout) — every prior Discover game (Calm Down Corner) uses emoji labels for exactly this reason. Feelings Detective's cues/feelings are genuine short phrases/sentences, which rendered illegible and overlapping under that CSS. Fixed with a scoped `.long-text`/`.long-text-list` opt-out in `brain-games.css` (Discover band only, doesn't touch Calm Down Corner or any other content). Also discovered the toggle-on/off multi-select state needed **no new CSS at all** — the existing `.selected` class already works as a plain toggle, since it never depended on `:disabled`.
