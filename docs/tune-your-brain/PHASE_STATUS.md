...
## Scenario Choice engine — scoped, not built

See `SCENARIO_CHOICE_ENGINE_SPIKE.md` for the full spike. Summary: no existing engine supports multi-select (every one of the 3 choice-based engines locks in a single choice on first click), which is exactly what genuine Self-awareness content (the "Feelings Detective" concept, §14.1) requires — the acceptance rule explicitly allows more than one plausible feeling per scenario. Recommended design: a two-stage-per-scenario shape reusing Evidence Hunt's proven pattern (notice cues, then pick feelings) but with both stages multi-select instead of single-select; narration built in from day one (not retrofitted later) since the primary use case is Discover band, same constraint that already forced Calm Down Corner onto Audio Choice instead of Branching Scenario; scoreless `onComplete`, per §5.6's evidence-discipline rule against "diagnosing" or "measuring CASEL competencies as a validated assessment." Needs two small `game-shell.js` additions (a toggleable multi-select button state, a submit control) — the SDK contract itself (`engine-sdk.js`) needs no changes.

**Two open decisions flagged, not defaulted** (see spike doc for full framing): (1) one-stage vs. two-stage per scenario — two-stage tracks "cue identification" as its own evidence dimension per the blueprint, one-stage is cheaper but loses that; (2) how many scenarios for a first slice.

## Next recommended step
Get a decision on the Scenario Choice spike's two open questions, then build `engine-scenario-choice.js` + a first Feelings Detective content pack (Discover band) against it — closing the last CASEL gap. In parallel or after: keep extending the Discover CSS direction further or move to sourcing real illustration, or pivot to admin-facing Tune Your Brain visibility / accessibility test tooling (both still zero). Worth deciding explicitly rather than defaulting.
