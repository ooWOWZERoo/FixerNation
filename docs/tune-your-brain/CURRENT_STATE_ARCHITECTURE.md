...
**Correction against this repo's own `CLAUDE.md`:** the root `CLAUDE.md` architecture table describes a `requireSiteAuth` middleware for site-user routes. It does not exist. `server/middleware/siteAuth.js` was a second, unused `requireSiteAuth` implementation deleted as dead code in a prior session (confirmed zero imports at the time). Site-user auth today is ad hoc `getSiteUser(req)` calls, repeated per route file (`brain-games.js`, `site-auth.js`, `social.js`, `parent-invite.js`, `school-invite.js`). This matters directly for the blueprint: any new Tune Your Brain route needs to either keep this ad hoc pattern or introduce the shared middleware the codebase has twice now not had — that's a real architecture decision, not a detail (see `LEADERSHIP_DECISIONS_REQUIRED.md`).
