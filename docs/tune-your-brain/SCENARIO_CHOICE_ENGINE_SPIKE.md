...
## Genuinely open, worth deciding explicitly (not defaulting)

1. **One-stage vs. two-stage per scenario.** The recommendation above (notice cues, then pick feelings, as two separate multi-select steps) most directly produces the blueprint's two named evidence dimensions as separate signals. A simpler one-stage alternative (skip the cue multi-select, go straight to feelings, fold cue-reasoning into the feedback text only) is cheaper to build and content-author, at the cost of losing "cue identification" as its own tracked evidence dimension. Worth a real choice before building, same as First Shift/The Post's content got confirmed before writing.
2. **How many scenarios for a first slice.** Every prior new-engine game shipped with a fixed round count (Calm Down Corner: 4 rounds). A first Feelings Detective slice would reasonably do the same — needs an actual number picked, not "as many as seems right."
3. **Engine name in code.** Blueprint calls it "Scenario Choice"; recommend registering it as `scenarioChoice` (`engine-scenario-choice.js`) to match the existing `branchingScenario`/`evidenceHunt` camelCase convention — flagging only because it's user-facing in code/docs from here on, not because there's a real alternative worth debating.

**Not started — this spike produces a design to build against, not working code.** Next step, once the two open decisions above are answered: write `engine-scenario-choice.js` against this contract, then a first Feelings Detective content pack (Discover band), following the same locally-verified-before-committing standard as every prior game this session.
