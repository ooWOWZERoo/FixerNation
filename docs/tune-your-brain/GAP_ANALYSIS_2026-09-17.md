...
**Update:** all 3 buildable-now games from the first batch (Calm Down Corner, First Shift, The Post) are live. **Update, 2026-09-17 (later still):** Explore's and Challenge's second SEL games (Team Pick, Group Chat Meltdown) are now also written, closing the Self-management gap in both bands — coded and pushed, see `PHASE_STATUS.md`'s Deploy queue for the deploy steps. Every band now has real Self-management coverage. **Update, 2026-09-17 (later still again):** the `Scenario Choice` engine is built (see `SCENARIO_CHOICE_ENGINE_SPIKE.md`) and **Feelings Detective** (Discover band, 4 scenarios) is written — the first genuine Self-awareness game in any band. Coded and pushed, not yet deployed. **Every one of the 5 CASEL competencies now has coverage in at least one band** — no more "zero coverage anywhere" gaps. Remaining SEL work is depth, not absence: Self-awareness/Self-management only exist in one band each so far; revisiting them for the other 3 bands (per this doc's own recommended-path item 4) is the natural next slice, not a hard blocker like the missing engine was.
