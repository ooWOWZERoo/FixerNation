...
- **Resolved 2026-09-21 — accessibility test tooling added site-wide, per the user's explicit "site-wide, not games-only" scope call.** `@axe-core/playwright` + `tests/e2e/accessibility-scan.spec.ts` (11 pages: public, admin, teacher, and 2 Tune Your Brain games). **Deliberately informational, not a CI gate** — logs every violation instead of failing, since establishing a real remediation plan for a site that's never been checked is separate, larger work than adding the tooling. First real run found real, cross-cutting issues on *every single page scanned*: missing `<main>` landmark (moderate), content not contained by landmarks (moderate), heading-order jumps (moderate), and — most significantly — `color-contrast` failures (serious, 7-27 nodes per page) everywhere. `admin-login.html`/`admin-dashboard.html` also have critical unlabeled form elements. **One real bug found and fixed in the same pass** (not just reported): `game-shell.js`'s shared `renderProgress()` — used by every single Tune Your Brain engine — had a `role="progressbar"` with no accessible name at all (an adjacent `sr-only` span existed but was never actually associated via `aria-labelledby`/`aria-label`). Fixed with `aria-labelledby`, verified locally. The bigger site-wide findings (contrast, landmarks, heading order) are **flagged, not fixed** — a real remediation plan is its own initiative, not something to fix blind across dozens of pages in this pass.
...
- ~~Accessibility test tooling (repo-wide, not games-specific).~~ Started 2026-09-21 — see this doc's own updated Finding 2 entry above.
...
**Update:** all 3 buildable-now games from the first batch (Calm Down Corner, First Shift, The Post) are live. **Update, 2026-09-17 (later still):** Explore's and Challenge's second SEL games (Team Pick, Group Chat Meltdown) are now also written, closing the Self-management gap in both bands — coded and pushed, see `PHASE_STATUS.md`'s Deploy queue for the deploy steps. Every band now has real Self-management coverage. **Update, 2026-09-17 (later still again):** the `Scenario Choice` engine is built (see `SCENARIO_CHOICE_ENGINE_SPIKE.md`) and **Feelings Detective** (Discover band, 4 scenarios) is written — the first genuine Self-awareness game in any band. Deployed and confirmed live 2026-09-21. **Every one of the 5 CASEL competencies now has coverage in at least one band** — no more "zero coverage anywhere" gaps. Remaining SEL work is depth, not absence.

**Corrected CASEL × band matrix, 2026-09-21 (pulled from every seed script's real `casel_competencies` value, not assumed):**

| | Discover | Explore | Challenge | Advance |
|---|---|---|---|---|
| Self-awareness | Feelings Detective | — | — | — |
| Self-management | Calm Down Corner | — | Group Chat Meltdown | The Post |
| Social awareness | — | Choice Quest | Decision Point | The Post |
| Relationship skills | — | Choice Quest | Decision Point | First Shift, The Post |
| Responsible decision-making | — | Choice Quest | Decision Point | First Shift |

This is a sharper picture than "each has 1 band, revisit evenly": **Discover was missing 3 of 5 competencies entirely** (Social awareness, Relationship skills, Responsible decision-making), and **Explore was missing 2 of 5** (Self-awareness, Self-management) — not a uniform 1-gap-per-competency situation. Discover's gap was prioritized as the more lopsided one.

**Update, 2026-09-21:** Discover's 3-competency gap is closed. **Sharing Circle** (Social awareness + Relationship skills) and **Helper's Choice** (Responsible decision-making) — both Audio Choice engine (same as Calm Down Corner; Branching Scenario still has zero audio support, so still not viable at Discover). Coded, verified locally, pushed. Remaining gap: Explore's Self-awareness/Self-management (2 of 5), not yet started.
