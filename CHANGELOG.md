...
- **Membership/subscription system** — coded but not yet deployed. Three consumer tiers (Free w/ Book, Monthly, Annual), three service-provider tiers (Monthly, Annual, 2D Education Program registration), and one Brand Ambassador tier, each admin-editable from a new "Memberships" page (plan CRUD + a Members tab for browsing/filtering/manually granting). `join.html` now renders its pricing cards from the plan catalog instead of static HTML, and two new public pages (`service-providers.html`, `brand-ambassador.html`) cover the other two member types — all three post to a new `/api/checkout/create-membership-session` endpoint that opens a real Stripe Checkout session (subscription mode with a trial period for recurring plans, one-time payment mode for the 2D Education registration). Every actual charge — the first one after a trial ends, and every renewal — creates its own order row via the `invoice.paid` webhook, so subscriptions show up in Orders/Financial Insights the same as any other purchase; the ongoing subscription record itself (status: trialing/active/past_due/cancelled/expired) lives in a separate `contact_memberships` table, visible on a contact's CRM record. Like the existing license Stripe checkout, this is **not live** until the admin's real Stripe API keys are configured — until then, plans not yet synced to Stripe show "Contact Us to Sign Up" instead of a working checkout button.
