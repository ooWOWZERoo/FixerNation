// Tune Your Brain, Phase 5 — adds Reading Detective as a real, assignable
...
// convention, "brain-{slug}.html"). Reading Detective's actual gameplay
// does NOT use brain_game_sessions/XP/badges (the legacy reward tables
// this catalog table is normally paired with) — it logs through the
...
async function main() {
  ...
  const [[existing]] = await conn.query('SELECT id FROM brain_games WHERE slug = ?', ['reading-detective']);
  if (existing) {
    console.log('Skipped: reading-detective already in brain_games catalog (id=' + existing.id + ')');
  } else {
    ...
    await conn.query(
      `INSERT INTO brain_games (name, slug, description, icon, primary_skill, display_order)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [
        'Reading Detective', 'reading-detective',
        'Read the clues in a short mystery, pick your answer, then prove it with evidence from the text. A Tune Your Brain pilot experience (Phase 5).',
        '🔍', 'Reading Comprehension', (maxOrder?.m || 0) + 1,
      ]
    );
    console.log('Added: reading-detective to brain_games catalog');
  }
