// Tune Your Brain, Phase 5 — adds Decision Point as a real, assignable
...
// convention, "brain-{slug}.html"). Decision Point's actual gameplay does
...
async function main() {
  ...
  const [[existing]] = await conn.query('SELECT id FROM brain_games WHERE slug = ?', ['decision-point']);
  if (existing) {
    console.log('Skipped: decision-point already in brain_games catalog (id=' + existing.id + ')');
  } else {
    ...
    await conn.query(
      ...
      [
        ...
        'Work through a real Issues-to-Answers cycle on a peer situation — recognize, understand impact, consider options, choose, and reflect. A Tune Your Brain pilot experience (Phase 5).',
        '🧭', 'Responsible Decision-Making', (maxOrder?.m || 0) + 1,
      ]
    );
    console.log('Added: decision-point to brain_games catalog');
  }
