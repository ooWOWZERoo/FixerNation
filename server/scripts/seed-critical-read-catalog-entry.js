// Catalog scaling slice (Phase 7/8) — adds Critical Read as a real,
// assignable entry in the classroom game-assignment flow, by extending
// the existing brain_games catalog table rather than building a parallel
// one.
//
// See seed-choice-quest-catalog-entry.js's header comment for the general
// pattern (reward_pipeline defaults to 'classroom_generic' automatically;
// run seed-classroom-completion-badges.js separately for its badge).
//
// Safe to re-run: skipped if a row with this slug already exists.
require('dotenv').config({ path: require('path').join(__dirname, '..', '.env') });
const mysql = require('mysql2/promise');

async function main() {
  const conn = await mysql.createConnection({
    host: process.env.DB_HOST, port: Number(process.env.DB_PORT || 3306),
    user: process.env.DB_USER, password: process.env.DB_PASSWORD, database: process.env.DB_NAME,
  });

  const [[existing]] = await conn.query('SELECT id FROM brain_games WHERE slug = ?', ['critical-read']);
  if (existing) {
    console.log('Skipped: critical-read already in brain_games catalog (id=' + existing.id + ')');
  } else {
    const [[maxOrder]] = await conn.query('SELECT MAX(display_order) AS m FROM brain_games');
    await conn.query(
      `INSERT INTO brain_games (name, slug, description, icon, primary_skill, display_order)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [
        'Critical Read', 'critical-read',
        'Judge real claims for evidence, bias, and source reasoning, then prove your judgment from the text. A Tune Your Brain catalog experience.',
        '🧐', 'Critical Comprehension', (maxOrder?.m || 0) + 1,
      ]
    );
    console.log('Added: critical-read to brain_games catalog');
  }

  await conn.end();
  console.log('Done.');
}

main().catch(err => { console.error(err); process.exit(1); });
