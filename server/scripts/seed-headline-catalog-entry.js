// Catalog scaling slice (Phase 7/8) — adds Headline as a real, assignable
...
async function main() {
  ...
  const [[existing]] = await conn.query('SELECT id FROM brain_games WHERE slug = ?', ['headline']);
  if (existing) {
    console.log('Skipped: headline already in brain_games catalog (id=' + existing.id + ')');
  } else {
    ...
    await conn.query(
      ...
      [
        'Headline', 'headline',
        'Read a short story, pick the most accurate headline, then prove it with evidence from the text. A Tune Your Brain catalog experience.',
        '📰', 'Media Literacy', (maxOrder?.m || 0) + 1,
      ]
    );
    console.log('Added: headline to brain_games catalog');
  }
