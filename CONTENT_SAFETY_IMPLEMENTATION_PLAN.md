...
- `@2toad/profanity` (npm)
- `nsfwjs` + `@tensorflow/tfjs-node` (npm) — **RESOLVED, two real host-specific incidents found and fixed during initial deploy (2026-08-25):**
  1. `npm install`'s script-execution allowlist (`npm approve-scripts`, already present on this host independent of this feature) blocked `tfjs-node`'s native install step by default — needed one explicit `npm approve-scripts @tensorflow/tfjs-node` (plus `core-js`/`sharp`, also flagged) before the native binding actually built.
  2. **A real production-crash risk, caught before real traffic hit it**: `tfjs-node`'s native runtime sizes its internal thread pool off the host's actual CPU count (32 cores) rather than this cPanel account's real CloudLinux LVE process/thread cap, which is enforced invisibly — `ulimit -a` and `/proc/self/limits` both report "unlimited." Uncapped, this hit `pthread_create() failed (EAGAIN)` and aborted the entire Node process with `SIGABRT` — a native crash a JS `try/catch` cannot contain, unlike every other failure mode this gateway was designed around. Fixed by forcing `TF_NUM_INTEROP_THREADS`/`TF_NUM_INTRAOP_THREADS`/`OMP_NUM_THREADS`/`DNNL_NUM_THREADS` to `1` before the module loads (`lib/safety/image.js`'s `applyHostCompatibilityFixes()`), verified via a live diagnostic session using a standalone script that called the real `classifyImageBuffer()` path directly — never routed through the live app while unverified.
  3. A second, unrelated compatibility gap surfaced once the crash was fixed: this `tfjs-node` build's core op-dispatch code still calls `util.isNullOrUndefined`, removed from Node.js itself years ago (the host runs Node 24). Fixed with a one-line polyfill in the same `applyHostCompatibilityFixes()`, since the function is pure and its historical behavior is trivial to replicate exactly.

  Given the crash was only reachable via the actual image-classification code path (not at module `require()` time — confirmed via `lib/safety/image.js`'s lazy-require design), and local image scanning shipped **off by default** behind `CONTENT_SAFETY_LOCAL_IMAGE_SCAN` (unset until this investigation completed), no real image upload hit this in production before the fix landed.
